...
class Config:
    ...
    # Ratio of features that are forced to be pseudo-invaraint or pseudo-equivariant by our special loss
    equivar_ratio = 0.0
    invar_ratio = 0.0
    equivar_power = 0.5
    invar_power = 2.0
    equivar_oriented = False

    # LRF related parameters
    ortho_power = 0.1
    first_n_lrf = 4
    lrf_up_factor = 2
    ...
    def save(self):
        ...
        with open(join(self.saving_path, 'parameters.txt'), "w") as text_file:
            ...
            text_file.write('\n')
            text_file.write('equivar_ratio = {:.6f}\n'.format(self.equivar_ratio))
            text_file.write('equivar_power = {:.6f}\n'.format(self.equivar_power))
            text_file.write('ortho_power = {:.6f}\n'.format(self.ortho_power))
            text_file.write('first_n_lrf = {:d}\n'.format(self.first_n_lrf))
            text_file.write('lrf_up_factor = {:d}\n'.format(self.lrf_up_factor))
            text_file.write('invar_ratio = {:.6f}\n'.format(self.invar_ratio))
            text_file.write('invar_power = {:.6f}\n'.format(self.invar_power))
            text_file.write('equivar_oriented = {:d}\n'.format(int(self.equivar_oriented)))
